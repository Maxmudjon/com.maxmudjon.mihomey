...
class Network extends EventEmitter {
	...
	findDeviceViaAddress(options) {
		if (!this.socket) {
			return new Error("Implementation issue: Using network without a reference");
		}
	...
	get socket() {
		if (!this._socket) {
			return new Error("Network communication is unavailable, device might be destroyed");
		}
...
class DeviceInfo {
	...
	enrich() {
		if (!this.id) {
			return new Error("Device has no identifier yet, handshake needed");
		}
		...
		if (!this.packet.token) {
			...
			promise = tokens.get(this.id).then(token => (this.token = Buffer.from(token, "hex")));
		} else {
