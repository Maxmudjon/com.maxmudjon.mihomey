...
class MiHub extends EventEmitter {
    ...
    MiInitialize(gateways) {
        ...
        if (gateways.length > 0) {
            ...
            gateways.forEach(gateway => {
                this.log(gateway)
                let data = { sid:gateway.mac.toLowerCase(), password:gateway.password.toUpperCase() }
    ...
    async controlLightHLS(sid, hue, saturation, brightness) {
        MiGateway.controlLightHLS({"sid": sid, "hue": hue, "saturation": saturation, "brightness": brightness})
    }
