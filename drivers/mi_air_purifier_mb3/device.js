      ...
      .then((device) => {
        ...
        this.device
          ...
          .then((result) => {
            ...
            this.setSettings({ buzzer: result[12] == "on" ? true : false });
    ...
    action.registerRunListener(async (args, state) => {
      try {
        miio
          ...
          .then((device) => {
            device
              .call("set_level_favorite", [args.range])
              .then(() => {
                this.log("Set 'set_level_favorite': ", args.range);
                device.destroy();
              })
              .catch((error) => {
