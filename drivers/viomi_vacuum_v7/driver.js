...
class XiaomiMijiaLDS extends Homey.Driver {
  onInit() {
    this.triggers = {
  ...
  onPair(socket) {
    let pairingDevice = {};
    pairingDevice.name = "Xiaomi Mijia LDS";
          ...
          device
            ...
            .then(value => {
              if (value.model == this.data.model) {
                pairingDevice.data.id = "MI:ML:DS:" + value.mac + ":MI:ML:DS";
                device
                  .call("get_prop", ["battary_life"])
                  .then(value => {
                    let result = {
                      battery: value[0]
                    };
                ...
                let result = {
                  notDevice: "It is not Xiaomi Mijia LDS"
                };
