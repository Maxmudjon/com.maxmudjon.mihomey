...
class YeelightJiaoyue650 extends Homey.Device {
  ...
  getYeelightStatus() {
    ...
    miio.device({ address: this.getSetting('deviceIP'), token: this.getSetting('deviceToken') })
      .then(device => {
        ...
        this.device.call("get_prop", ["power", "bright", "ct", "color_mode", "bg_power", "bg_bright", "bg_rgb", "bg_ct", "bg_lmode"])
          .then(result => {
  ...
  registerHueLevel(name) {
    this.registerCapabilityListener(name, async (value) => {
      ...
      this.device.call('bg_set_rgb', [argbToSend])
  ...
  registerBGOnOffButton(name) {
    this.registerCapabilityListener(name, async (value) => {
      this.device.call('bg_set_power', [value ? 'on' : 'off'])
        .then(() => this.log('Sending ' + name + ' commmand: ' + value))
  ...
  registerBGDimLevel(name) {
    this.registerCapabilityListener(name, async (value) => {
      if (value * 100 > 0) {
        this.device.call('bg_set_bright', [value * 100])
          .then(() => this.log('Sending ' + name + ' commmand: ' + value))
  ...
  registerBGLightTemperatureLevel(name) {
    this.registerCapabilityListener(name, async (value) => {
      let color_temp = this.denormalize(value, 1700, 6500);
      this.device.call('bg_set_ct_abx', [color_temp, "smooth", 500])
        .then(() => this.log('Sending ' + name + ' commmand: ' + color_temp))
