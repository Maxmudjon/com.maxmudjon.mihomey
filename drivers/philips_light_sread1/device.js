...
class YeelightWhiteBulb extends Homey.Device {
  ...
  getYeelightStatus() {
    ...
    miio.device({ address: this.getSetting('deviceIP'), token: this.getSetting('deviceToken') })
      .then(device => {
        ...
        this.device.call("get_prop", ["power", "bright", "ambstatus", "eyecare"])
          .then(result => {
  ...
  registerEyeCareButton(name) {
    this.registerCapabilityListener(name, async (value) => {
      this.device.call('set_eyecare', [value ? 'on' : 'off'])
        .then(() => this.log('Sending ' + name + ' commmand: ' + value))
  ...
  registerAmbilightButton(name) {
    this.registerCapabilityListener(name, async (value) => {
      this.device.call('enable_amb', [value ? 'on' : 'off'])
        .then(() => this.log('Sending ' + name + ' commmand: ' + value))
  ...
  registerAmbilightLevel(name) {
    this.registerCapabilityListener(name, async (value) => {
      if (value * 100 > 0) {
        this.device.call('set_amb_bright', [value * 100])
          .then(() => this.log('Sending ' + name + ' commmand: ' + value))
  ...
  registerAmbilightLevelAction(name, action) {
    action.action.registerRunListener(async (args, state) => {
      this.device.call('set_amb_bright', [args.level * 100])
  ...
  registerEyecareSceneAction(name, action) {
    action.action.registerRunListener(async (args, state) => {
      this.device.call('set_user_scene', [args.scene])
