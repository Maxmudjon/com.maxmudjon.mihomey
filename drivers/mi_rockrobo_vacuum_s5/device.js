      ...
      .then(device => {
        ...
        this.device
          ...
          .then(result => {
            that.setCapabilityValue(
              "onoff",
              result[0]["state"] === 5 ? true : false
            );
            that.setCapabilityValue("dim", parseInt(result[0]["fan_power"]));
            that.setCapabilityValue(
              "measure_battery",
              parseInt(result[0]["battery"])
            );
            that.setCapabilityValue(
              "alarm_battery",
              parseInt(result[0]["battery"]) <= 20 ? true : false
            );
          ...
          .catch(error =>
            that.log("Sending commmand 'get_status' error: ", error)
          );
        ...
        this.device
          ...
          .then(result => {
            ...
            let mainBrushCurrentLife = parseInt(
              result[0]["main_brush_work_time"]
            );
            let mainBrushLifeTimePercent =
              (mainBrushCurrentLife / mainBrushLifeTime) * 100;
            let sideBrushLifeTime = 720000;
            let sideBrushCurrentLife = parseInt(
              result[0]["side_brush_work_time"]
            );
            let sideBrushLifeTimePercent =
              (sideBrushCurrentLife / sideBrushLifeTime) * 100;
            let filterLifeTime = 540000;
            let filterCurrentLife = parseInt(result[0]["filter_work_time"]);
            let filterLifeTimePercent =
              (filterCurrentLife / filterLifeTime) * 100;
            let sensorLifeTime = 108000;
            let sensorCurrentLife = parseInt(result[0]["main_brush_work_time"]);
            let sensorLifeTimePercent =
              (sensorCurrentLife / sensorLifeTime) * 100;

            that.setSettings({
              main_brush_work_time: parseInt(mainBrushLifeTimePercent) + "%"
            });
            that.setCapabilityValue(
              "alarm_main_brush_work_time",
              100 - parseInt(mainBrushLifeTimePercent) <=
                this.getSetting("alarm_threshold")
                ? true
                : false
            );
            that.triggerFlow(
            ...
            that.setSettings({
              side_brush_work_time: parseInt(sideBrushLifeTimePercent) + "%"
            });
            that.setCapabilityValue(
              "alarm_side_brush_work_time",
              100 - parseInt(sideBrushLifeTimePercent) <=
                this.getSetting("alarm_threshold")
                ? true
                : false
            );
            ...
            that.setSettings({
              filter_work_time: parseInt(filterLifeTimePercent) + "%"
            });
            that.setCapabilityValue(
              "alarm_filter_work_time",
              100 - parseInt(filterLifeTimePercent) <=
                this.getSetting("alarm_threshold")
                ? true
                : false
            );
            ...
            that.setSettings({
              sensor_dirty_time: parseInt(sensorLifeTimePercent) + "%"
            });
            that.setCapabilityValue(
              "alarm_sensor_dirty_time",
              100 - parseInt(sensorLifeTimePercent) <=
                this.getSetting("alarm_threshold")
                ? true
                : false
            );
          ...
          .catch(error =>
            that.log("Sending commmand 'get_consumable' error: ", error)
          );
        ...
        this.device
          ...
          .then(result => {
            that.setSettings({
              total_work_time: that.convertMS(parseInt(result[0]))
            });
            that.setSettings({
              total_cleared_area: parseInt(result[1] / 1000000)
            });
            that.setSettings({ total_clean_count: parseInt(result[2]) });
          })
          .catch(error =>
            that.log("Sending commmand 'get_clean_summary' error: ", error)
          );
  ...
  registerOnOffButton(name) {
    this.registerCapabilityListener(name, async value => {
      this.device
        ...
        .catch(error =>
          this.log("Sending commmand 'app_start' or 'app_pause' error: ", error)
        );
    });
  }
      ...
      if (speed > 0) {
        if (speed > 1 && speed <= 38) {
          this.device
            ...
            .catch(error =>
              this.log("Sending commmand 'set_level_favorite' error: ", error)
            );
    ...
    this.registerCapabilityListener(name, async value => {
      if (value == "cleaning") {
        this.device
          ...
          .then(() => this.log("Sending " + name + " commmand: " + value))
          .catch(error =>
            this.log("Sending commmand 'app_start' error: ", error)
          );
        ...
        this.device
          ...
          .then(() => this.log("Sending " + name + " commmand: " + value))
          .catch(error =>
            this.log("Sending commmand 'app_spot' error: ", error)
          );
        ...
        this.device
          ...
          .then(() => this.log("Sending " + name + " commmand: " + value))
          .catch(error =>
            this.log("Sending commmand 'app_charge' error: ", error)
          );
        ...
        this.device
          ...
          .then(() => this.log("Sending " + name + " commmand: " + value))
          .catch(error =>
            this.log("Sending commmand 'app_pause' error: ", error)
          );
      }
    });
  }
