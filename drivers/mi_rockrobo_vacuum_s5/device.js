...
class MiVacuumCleanerV2 extends Homey.Device {
  ...
  async initialize() {
    ...
    this.registerCapabilityListener("button.consumable", async () => {
      return new Promise(async (resolve, reject) => {
        try {
          const result = await this.device.call("reset_consumable", ["main_brush_work_time", "side_brush_work_time", "filter_work_time", "sensor_dirty_time"]);
          resolve(result);
        } catch (error) {
          reject(error);
        }
      ...
      .then((device) => {
        ...
        this.device
          ...
          .then((result) => {
            ...
            this.updateCapabilityValue("dim", this.speeds[result[0].fan_power]);
        ...
        this.device
          ...
          .then((result) => {
            ...
            this.setSettings({ main_brush_work_time: Math.abs(parseInt(mainBrushLifeTimePercent - 100)) + "%" });
            ...
            this.setSettings({ side_brush_work_time: Math.abs(parseInt(sideBrushLifeTimePercent - 100)) + "%" });
            ...
            this.setSettings({ filter_work_time: Math.abs(parseInt(filterLifeTimePercent - 100)) + "%" });
            ...
            this.setSettings({ sensor_dirty_time: Math.abs(parseInt(sensorLifeTimePercent - 100)) + "%" });
  ...
  updateTimer(interval) {
    ...
    this.updateInterval = setInterval(() => {
      this.device
        ...
        .then((result) => {
          this.updateCapabilityValue("onoff", result[0]["state"] === 5 ? true : false);
          this.updateCapabilityValue("dim", this.speeds[parseInt(result[0]["fan_power"])]);
          this.updateCapabilityValue("measure_battery", parseInt(result[0]["battery"]));
          this.updateCapabilityValue("alarm_battery", parseInt(result[0]["battery"]) === 20 ? true : false);
